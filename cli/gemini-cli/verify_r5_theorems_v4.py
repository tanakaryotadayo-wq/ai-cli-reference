...
# matplotlib をオプション依存性に緩和 (v5.2 - R5_v4)
try:
...
def test_l3_routing_v5_1():
    ...
    # --- R5_v4 修正: OPF（直交射影）の直交補空間類似度分岐テスト ---
    print("--- Testing L3 OPF Specific Branch ---")
    # n_max=2 に設定し、ツール2件登録で混雑度 rho = 1.0 とする。これにより DQS をスルーして OPF へ引き渡す
    router_opf = L3OrthogonalExclusionRouter(n_max=2, entropy_crit=0.5, rho_threshold=3.0)
    
    # ツール登録（半径を 1.6 とし、クエリが確実に両方の許容範囲内に入るようにする）
    ...
    # クエリ q_opf: x=0.8, y=0.6, z=0.0 (x軸の v_best に近く、z軸の tool_alternative とは直交)
...
if __name__ == "__main__":
    print("=" * 50)
    print("Starting R5 Theorems Verification Suite (v5.2 - R5_v4)")
    print("=" * 50)
    ...
    print("=" * 50)
    print("All R5 theorems verified successfully (v5.2 - R5_v4)!")
    print("=" * 50)
