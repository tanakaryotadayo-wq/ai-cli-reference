...
class LoopFreeFixedPointMonitor:
    ...
    def evaluate_transition(self, session_id: uuid.UUID, state_hash: str, errors: int, test_failures: int, risk: float, retry: int, api: float) -> AgentStateVector:
...
def test_l2_loop_free_fixed_point_monitor():
    ...
    with Session(engine) as session:
        ...
        state_1 = monitor.evaluate_transition(
            session_id=session_id, state_hash="hash_normal_A", errors=3, test_failures=2, risk=0.1, retry=10, api=100.0
        )
        print(f"Step 1 - Potential: {state_1.extended_potential_value}")
        
        state_2 = monitor.evaluate_transition(
            session_id=session_id, state_hash="hash_normal_B", errors=2, test_failures=1, risk=0.1, retry=9, api=90.0
        )
        print(f"Step 2 - Potential: {state_2.extended_potential_value}")
        assert state_2.extended_potential_value < state_1.extended_potential_value, "Potential must strictly decrease."
        
        try:
            monitor.evaluate_transition(
                session_id=session_id, state_hash="hash_normal_A", errors=2, test_failures=1, risk=0.1, retry=8, api=80.0
            )
            assert False, "Should raise LoopDetectedException due to duplicate state hash."
        ...
        state_stuck_1 = monitor.evaluate_transition(
            session_id=session_id_stuck, state_hash="hash_stuck_A", errors=1, test_failures=1, risk=0.0, retry=10, api=100.0
        )
        try:
            monitor.evaluate_transition(
                session_id=session_id_stuck, state_hash="hash_stuck_B", errors=1, test_failures=1, risk=0.0, retry=9, api=90.0
            )
            assert False, "Should raise LoopDetectedException due to non-decreasing potential value."
        except LoopDetectedException as e:
...
class AutoimmuneSandbox:
    ...
    def update_effective_risk(self, i_raw: float, has_sensitive_action: bool = False) -> float:
    ...
    def should_force_super_degraded(self, i_eff: float, slack_time: float, t_rec: float) -> bool:
...
def test_l4_autoimmune_sandbox():
    ...
    assert not sandbox.should_force_super_degraded(i_eff=0.9, slack_time=300.0, t_rec=100.0)
...
def test_l3_routing_v5_1():
    ...
    router_opf = L3OrthogonalExclusionRouter(n_max=2, entropy_crit=0.5, rho_threshold=3.0)
    router_opf.register_tool("tool_best", np.array([1.0, 0.0, 0.0]), radius=1.6, namespace="sys")
    router_opf.register_tool("tool_alternative", np.array([0.0, 0.0, 1.0]), radius=1.6, namespace="sys")
    ...
    assert res_opf == "tool_best", "OPF must filter competitors and resolve to the best candidate."
...
def residue(x: str, key: str) -> dict:
    ...
    if key == "PermissionError":
        ...
        if "100%正しい" in x_clean or "絶対的" in x_clean:
            res["overstatement"] = "Claim of 100% mathematical correctness in Lean without real-world context validation"
...
def test_tunnel_research_compiler():
    ...
    item = TunnelItem(raw=memo, key=key, alpha=alpha, invariants=inv, source="research_notes")
    
    print(f"Canonical Key: {item.key}")
    print(f"Residual alpha: {item.alpha}")
    print(f"Invariants N(x): {item.invariants}")
    
    assert item.key == "FORMAL_VERIFICATION_LIMIT"
    assert "danger" in item.alpha, "Unverified claim must be captured as danger residue."
    assert item.invariants["risk_level"] >= 0.8, "Must yield high risk level for unverified mathematical overstatements."
...
def test_tunnel_error_resolver():
    ...
    item = TunnelItem(raw=error_log, key=key, alpha=alpha, invariants=inv, source="cli_stderr")
    ...
    assert "Proposed command" in solution
...
def test_tunnel_clipboard():
    ...
    item = TunnelItem(raw=copied_url, key=key, alpha=alpha, invariants=inv, source="clipboard")
...
def test_l2_l7_l9_unified_potential():
    ...
    # 状態、有向閉路有無、残余 Slack Time, 残余 L9 バジェット
    # Psi = Potential(v_t) + alpha_loop * I(loop) + gamma_L7 * (1/L(t)) + beta_L9 * (5 - k)
    alpha_loop = 1000.0
    gamma_l7 = 50.0
    beta_l9 = 10.0
    
    def calculate_psi_unified(errors: int, is_loop: bool, slack_time: float, k_mutate: int) -> float:
        phi = 10.0 * errors
    ...
    # 1. 正常収束ルート (エラー減少、ループなし、十分な時間、変異減少)
    psi_start = calculate_psi_unified(errors=3, is_loop=False, slack_time=200.0, k_mutate=5)
    psi_next = calculate_psi_unified(errors=1, is_loop=False, slack_time=190.0, k_mutate=4)
    print(f"Normal Trace - Start Psi: {psi_start:.4f}, Next Psi: {psi_next:.4f}")
    assert psi_next < psi_start, "Potential must strictly decrease in normal paths."

    # 2. ループ発生時の検知
    psi_loop = calculate_psi_unified(errors=1, is_loop=True, slack_time=180.0, k_mutate=3)
    print(f"Loop Detected Trace - Psi: {psi_loop:.4f}")
    assert psi_loop > 1000.0, "Psi must spike upon loop detection."

    # 3. L7 時間枯渇による相転移
    psi_timeout = calculate_psi_unified(errors=1, is_loop=False, slack_time=0.01, k_mutate=3)
    print(f"Time Exhaustion Trace - Psi: {psi_timeout:.4f}")
    assert psi_timeout > 4000.0, "Psi must spike when time budget falls near zero."

    # 4. L9 修復変異限界による相転移
    psi_limit = calculate_psi_unified(errors=1, is_loop=False, slack_time=150.0, k_mutate=0)
    print(f"Mutation Limit Trace - Psi: {psi_limit:.4f}")
    assert psi_limit > psi_start, "Psi must reflect mutation budget exhaustion."
...
if __name__ == "__main__":
    print("=" * 50)
    print("Starting R5 Theorems Verification Suite (v7 - Residual Cache)")
    print("=" * 50)
    ...
    print("=" * 50)
    print("All theorems verified successfully (v7 - Residual Cache)!")
    print("=" * 50)
