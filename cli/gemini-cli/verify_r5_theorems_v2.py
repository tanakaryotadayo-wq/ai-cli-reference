...
# matplotlib をオプション依存性に緩和 (v5.2 - R5_v2)
try:
...
class AgentStateVector(SQLModel, table=True):
    ...
    timestamp: float = Field(default_factory=time.time if 'time' in globals() else 0.0)
...
def test_l2_loop_free_fixed_point_monitor():
    ...
    with Session(engine) as session:
        ...
        # 2. 循環（以前に遷移した状態への逆戻り）-> LoopDetectedException の送出を検証
        try:
        ...
        # 3. 膠着（改善が見られずポテンシャルが下がらない）-> LoopDetectedException の送出を検証
...
if __name__ == "__main__":
    print("=" * 50)
    print("Starting R5 Theorems Verification Suite (v5.2 - R5_v2)")
    print("=" * 50)
    ...
    print("=" * 50)
    print("All R5 theorems verified successfully (v5.2 - R5_v2)!")
    print("=" * 50)
