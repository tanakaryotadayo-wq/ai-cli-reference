...
def test_l3_routing_v8_scaling():
    ...
    assert mode_v8 in ["dqs_resolved", "opf_resolved", "hnpt_resolved", "default_router"]
    print(">> L3 Routing v8 Scaling Integration Test: PASS\n")


# ==========================================
# 7. 全レイヤー (L1-L9) 結合コヒーレンスループ数値検証 (v9)
# ==========================================
def test_full_9_layer_coherence_loop():
    ...
    def calculate_psi_unified(errors: int, is_loop: bool, slack_time: float, k_mutate: int) -> float:
        phi = 100.0 * errors  # v9 調整: errors 減少の重みを 100.0 に引き上げる
        term_loop = alpha_loop if is_loop else 0.0
        term_l7 = gamma_l7 * (1.0 / max(slack_time, 1e-9))
        term_l9 = beta_l9 * (5 - k_mutate)  # v9 調整: k_mutate 減少のペナルティを 5.0 に下げる
        return phi + term_loop + term_l7 + term_l9
    ...
    print(f"Cycle 0 - Psi: {psi_curr:.4f} (errors={errors}, slack={slack_time:.1f}, k={k_mutate})")
    ...
    for cycle in range(1, 10):
        ...
        print(f"Cycle {cycle} - Psi: {psi_next:.4f} (errors={errors}, slack={slack_time:.1f}, k={k_mutate})")
        
        # コヒーレンス調整後、正常修復時には Psi が狭義単調減少することを確認
        if cycle < 3:
            assert psi_next < psi_history[-2], f"Psi must decrease on successful recovery. Prev: {psi_history[-2]:.4f}, Curr: {psi_next:.4f}"
            
        if slack_time <= 10.0 or k_mutate == 0:
            print(f"Phase transition condition met at cycle {cycle}. Terminating simulation loop.")
            break
...
if __name__ == "__main__":
    print("=" * 50)
    print("Starting R5 Theorems Verification Suite (v9 - Coherence Loop)")
    print("=" * 50)
    ...
    print("=" * 50)
    print("All theorems verified successfully (v9 - Coherence Loop)!")
    print("=" * 50)
