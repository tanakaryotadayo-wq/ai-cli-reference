"""Wick-Loop Instanton Theorem V6.5.0 (Theorem V2.2 / Governance V6.0) for AI Agent Search and Reasoning.
...
try:
    ...
    # エージェントが保持する状態の定義 (V6.5 対応、RNGを因果分離)
    ...
    def reasoning_node(state: InstantonAgentState) -> Dict[str, Any]:
        """推論を行うメインのノード"""
        env_seed = state.get("env_seed", 42)
        ...
        next_env_seed = int(rng.integers(0, 1000000))
    ...
    def tunnel_check_node(state: InstantonAgentState) -> Dict[str, Any]:
        ...
        tunnel_seed = state.get("tunnel_seed", 17)
        rng = np.random.default_rng(tunnel_seed)
        ...
        success, next_state, trigger_safe, new_failed_attempts = tunnel.attempt_tunnel(
            vectors, wells, failed_attempts=failed_attempts, max_attempts=9, safe_mode=safe_mode
        )

        next_tunnel_seed = int(rng.integers(0, 1000000))
    ...
    def safe_mode_recovery_node(state: InstantonAgentState) -> Dict[str, Any]:
        ...
        if wells:
            ...
            # 目的地が空リストの場合、強いノイズ（0.2）を加えて膠着点から引き離す
...
if __name__ == "__main__":
    logger.info("instanton_test_start", msg="=== Wick-Loop Instanton Theorem V6.5.0 動作テスト ===")
    ...
    # 3. セーフモード状態移行テスト (max_attempts=9)
    logger.info("safe_mode_test_start", msg="--- セーフモード状態移行テスト (V6.5.0) ---")
    tunnel.hbar = 0.1
    failed_attempts = 0
    safe_triggered = False
    for attempt in range(1, 15):
        success, next_state, trigger_safe, failed_attempts = tunnel.attempt_tunnel(
            current_trajectory, destination_wells[:2], failed_attempts=failed_attempts, max_attempts=9
        )
        logger.info(
            "attempt_result",
            attempt=attempt,
